...
async def _normalize_tool_result(result):
    if isinstance(result, list) and len(result) == 1 and isinstance(result[0], dict):
        # StructuredTool returns a list of LangChain tool outputs
...
async def _call(client, name: str, args: dict = {}):
    ...
    result = await _normalize_tool_result(result)
    if name == "list_smart_devices" and isinstance(result, dict):
        return [result]
    return result
...
async def run_tests():
    ...
    # ------------------------------------------------------------------
    print("[ TEST 5 ] get_device_status (after turn_on)")
    try:
        result = await _call(client, "get_device_status")
        assert result["is_on"] == True, f"Expected is_on=True, got {result['is_on']}"
    ...
    # ------------------------------------------------------------------
    print("[ TEST 7 ] get_device_status (after turn_off)")
    try:
        result = await _call(client, "get_device_status")
        assert result["is_on"] == False, f"Expected is_on=False, got {result['is_on']}"
